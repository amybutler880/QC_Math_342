...
```{r}
rf_mod = randomForest(data.frame(x=x_train), y=y_train, ntree=1, replace=FALSE, sampsize=n, nodesize=node_sizes[which.min(SE_by_node_sizes)])
...
find the bootstrap s_e for a RF model using 1, 2, 5, 10, 20, 30, 40, 50, 100, 200, 300, 400, 500, 1000 trees. If you are using the `randomForest` package, you can calculate oob residuals via `e_oob = y_train - rf_mod$predicted`. Plot it.
...
for (i in 1:length(num_trees)){
  rf_mod= randomForest(price ~. , data=diamonds_samp, ntree=num_trees[i], ntry=ncol(diamonds_samp)-1)
